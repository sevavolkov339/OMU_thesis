// запускаем чёрный fade-переход только для обычных дверей — для последней двери сегмента
// (переход на титульный экран) fade не нужен, там просто тишина и пауза.
// Музыка и звук в этот момент разные для обоих случаев.
if (!fade_started) {
    fade_started = true;
    if (!is_segment_final) {
...
if (alpha < 0.05) {
    if (is_segment_final) {
        // без fade-перехода — сразу отложенный переход на титульный экран (музыка уже остановлена там)
