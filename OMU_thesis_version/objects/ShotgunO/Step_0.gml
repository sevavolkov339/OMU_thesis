if (held) {
    speed = 0;
    if (anim_playing) shotgun_reset_idle();
    exit;
}

if (shotgun_bounce_cooldown > 0) shotgun_bounce_cooldown--;
...
if (teleporting) {
    ...
    visual_angle = lerp(visual_angle, 0, 0.3);
    image_angle = 0;
    spin = 0;
...
with (BulletBounceO) {
    if (id != other.id && speed > 1.5 && other.speed > 1.5) {
        ...
        if (_dist < _combined_radius) {
            ...
            if (other.object_index == ShotgunO) {
                other.bounce_happened = true;
                other.collision_occurred = true;
            }
            if (object_index == ShotgunO) {
                bounce_happened = true;
                collision_occurred = true;
            }
            if (!touching_bullet) {
...
if (bounce_happened) shotgun_on_bounce();

// вращение зависит от скорости (только визуально, маска остаётся прямой)
spin_angle += speed * spin_rate * spin_dir;
visual_angle = floor(spin_angle / spin_step) * spin_step;
image_angle = 0;
