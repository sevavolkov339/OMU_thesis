...
// "насест" — своя точка возле игрока, за которую ворона держится, а не летает по кругу.
// Сторона (лево/право от игрока) не фиксирована — переключается в Step_0, когда игрок
// обгоняет ворону и оказывается по другую сторону от неё
...
// лёгкое покачивание в стороны на месте (не по кругу, просто дрожь около своей точки)
...
return_speed = 1.6;
arrive_dist = 4; // насколько близко нужно долететь, чтобы считать "вернулась домой" / "долетела до врага"
