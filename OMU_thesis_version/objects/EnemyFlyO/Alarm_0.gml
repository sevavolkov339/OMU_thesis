...
// при flow field путь не ищем, направление берётся в Step_0
var _pv = instance_exists(PipelineValidationO);
if (_pv && PipelineValidationO.cfg_pathfinder == "flow") exit;

if instance_exists(PlayerBallerO){

	// a* по coarse_grid, тот же код что и в тестовых объектах
	var _pf_t0 = get_timer();
	...
	var _ex = clamp(floor(PlayerBallerO.x / _cs), 0, _gw - 1);
	var _ey = clamp(floor(PlayerBallerO.y / _cs), 0, _gh - 1);
	...
	while (!ds_priority_empty(_open)) {
		...
		for (var d = 0; d < 8; d++) {
			...
			// диагональный шаг нельзя срезать через угол стены - если хоть одна из двух
			if (d >= 4 && (_grid[_nx][_cy] == 1 || _grid[_cx][_ny] == 1)) continue;
	...
	if (_pv) PipelineValidationO.add_path_time(get_timer() - _pf_t0);
